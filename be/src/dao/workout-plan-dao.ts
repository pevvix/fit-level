...
export class WorkoutPlanDAO {
  ...
  async create(plan: WorkoutPlanEntity): Promise<void> {
  ...
  async update(plan: WorkoutPlanEntity): Promise<void> {
  ...
  async findById(id: string): Promise<WorkoutPlanEntity | undefined> {
    return new Promise((resolve, reject) => {
      this.db.get(`SELECT * FROM workout_plan WHERE id = ?`, [id], (err, row) => {
        err ? reject(err) : resolve(row as WorkoutPlanEntity);
      });
    });
  }

  async getAll(): Promise<WorkoutPlanEntity[]> {
    return new Promise((resolve, reject) => {
      this.db.all(`SELECT * FROM workout_plan`, [], (err, rows) => {
        err ? reject(err) : resolve(rows as WorkoutPlanEntity[]);
      });
    });
  } 
